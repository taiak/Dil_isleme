...
module DS
  ...
  class ::String
    ...
    VOWELS =  /[aâeiîioouuAÂEIIÎOOUU]/
    CONSONANT = /[bccdfgghjklmnprsstvyzBCCDEFGHJKLMNPRSSTVYZ]/
    ...
    # CHARS = "AÂBCCDEFGGHIÎIJKLMNOOPQRSSTUÛUVWYZaâbccdefgghiîijklmnoopqrsstuûuvwyz".freeze
    ...
    # bit icin heceleme saglar. sessiz harfler kontrol edilmez
    ...
    def each_count?(str)
      str.freq.collect { |k, v| str_count?(k, v) }.all?
    end
  ...
  class ::Array
    ...
    # verilen elemanlari stringe cevirir
    # FIXME: false degeri icin "false uretiyor"
    def to_str!
      collect!(&:to_s)
    end

    # icinde num sayisi kadar str olan elemanlari secer
    def search_rep?(str, num)
      select { |word| word.str_count?(str, num) }
    end

    # frequency of elements. Return hash
    def freq
      each_with_object(Hash.new(0)) { |val, hsh| hsh[val] += 1; }
    end

    # verilen stringin icerigi olan elemanlari secer
    # stringin icideki elemanlari iceren elemanlari doner
    # elemanlar sirali olmak zorunda degildir
    def search_chars?(str)
      select { |word| word.each_count?(str) }
    end
    ...
    # verilen dizideki elemanlari sayar ve [adi, sayisi] seklinde uniq bir liste verir. dizi doner
    def rep_count
      each_with_object(Hash.new(0)) { |key, hsh| hsh[key] += 1 }.sort { |sym, rep| rep[1].to_i <=> sym[1].to_i }
    end
